...
def require_configured_rtsp_urls(left_rtsp: str, right_rtsp: str, *, context: str) -> None:
    ...
    if missing:
        raise RuntimeSiteConfigError(
            f"{context} requires configured RTSP URLs. Update config/runtime.json or set "
            f"HOGAK_LEFT_RTSP and HOGAK_RIGHT_RTSP. Missing/placeholder fields: {', '.join(missing)}"
        )
