...
RUNTIME_GEOMETRY_SUPPORTED_RESIDUAL_MODELS = frozenset({"rigid"})
...
def _normalize_geometry_model(model: Any) -> str:
    text = str(model or "virtual-center-rectilinear").strip().lower().replace("_", "-")
    if text in {
        "virtual-center-rectilinear",
...
def _normalize_residual_model(model: Any, *, geometry_model: str = "") -> str:
    ...
    if text in {"mesh", "virtual-center-rectilinear-mesh"}:
        return "mesh"
    if text in {"rigid", "virtual-center-rectilinear-rigid"}:
        return "rigid"
...
def _ensure_v2_shape(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload["schema_version"] == 1:
        raise ValueError(
            "schema_version=1 runtime geometry artifacts are no longer supported; regenerate a virtual-center rigid artifact"
        )
    ...
    if payload["geometry"]["model"] not in RUNTIME_GEOMETRY_SUPPORTED_MODELS:
        raise ValueError(
            "unsupported runtime geometry model; only virtual-center-rectilinear-rigid artifacts are allowed"
        )
    ...
    if payload["geometry"]["residual_model"] not in RUNTIME_GEOMETRY_SUPPORTED_RESIDUAL_MODELS:
        raise ValueError(
            "unsupported runtime geometry residual model; only rigid artifacts are allowed on the product path"
        )
...
def build_runtime_geometry_artifact(
    *,
    ...
    geometry_model: str = "virtual-center-rectilinear",
    warp_model: str = "virtual-center-remap",
    alignment_model: str = "rigid",
    ...
    geometry_model = _normalize_geometry_model(geometry_model or "virtual-center-rectilinear")
    if geometry_model not in RUNTIME_GEOMETRY_SUPPORTED_MODELS:
        raise ValueError(
            "unsupported runtime geometry model; only virtual-center-rectilinear artifacts can be built"
        )
    resolved_residual_model = _normalize_residual_model(residual_model, geometry_model=geometry_model)
    if resolved_residual_model not in RUNTIME_GEOMETRY_SUPPORTED_RESIDUAL_MODELS:
        raise ValueError(
            "unsupported runtime geometry residual model; only rigid artifacts can be built"
        )
...
def runtime_geometry_model(artifact: dict[str, Any]) -> str:
    ...
    if not isinstance(geometry, dict):
        return "virtual-center-rectilinear"
    return _normalize_geometry_model(geometry.get("model") or "virtual-center-rectilinear")
...
def runtime_geometry_effective_residual_model(artifact: dict[str, Any]) -> str:
    return runtime_geometry_requested_residual_model(artifact)
