...
private:
    ...
    using SelectedPair = PairSchedulerSelectedPair;

    struct EngineInputState {
        hogak::input::FfmpegRtspReader left_reader{};
        hogak::input::FfmpegRtspReader right_reader{};
    };
    ...
    struct PreparedExecutionInput {
        ...
        bool gpu_nv12_fast_path = false;
    };

    struct BufferedInfoCacheState {
        bool valid = false;
    ...
    void invalidate_pair_selection_cache_locked();
    void update_metrics_locked();
    void refresh_sampled_metrics_locked(
        const hogak::input::ReaderSnapshot& left,
        const hogak::input::ReaderSnapshot& right,
        std::int64_t now_arrival_ns,
        std::int64_t now_source_wallclock_ns);
    void refresh_output_metrics_locked(std::int64_t now_arrival_ns);
    void maybe_restart_stale_readers_locked();
    ...
    bool prepare_execution_inputs_locked(
    ...
    PairSchedulerState pair_scheduler_state_{};
    ...
    std::int64_t last_metrics_refresh_ns_ = 0;
    ...
    BufferedInfoCacheState left_buffered_infos_cache_state_{};
    BufferedInfoCacheState right_buffered_infos_cache_state_{};
